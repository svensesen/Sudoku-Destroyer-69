...
class SudokuAI(object):
    ...
    def compute_best_move(self, game_state: GameState) -> None:
        ...
        global_m = game_state.m
        global_n = game_state.n
        ...
        root = Node(None, 1, possible_moves, empty_squares, 0, 0)
...
def solve_sudoku(board: SudokuBoard, possible_moves: list, numbers_left: dict) -> SudokuBoard:
    ...
    while change_made == True:
        ...
        for move in possible_moves:
            possibilities = set(numbers_left["rows"][move[0]] & numbers_left["columns"][move[1]] & numbers_left["regions"][square2region(move, board.m, board.n)])
            ...
            if len(possibilities) == 1:
                ...
                try:
                    ...
                    numbers_left["regions"][square2region(move, board.m, board.n)].remove(number)
                except:
                    return -1
    ...
    while change_made == True:
        ...
        for move in move_possibilities:
            ...
            region_possibilities[square2region(move, board.m, board.n)].extend(move_possibilities[move])
        ...
        for move in possible_moves:
            for number in move_possibilities[move]:
                if (row_possibilities[move[0]].count(number) == 1) or (column_possibilities[move[1]].count(number) == 1) or (region_possibilities[square2region(move, board.m, board.n)].count(number) == 1):
    ...
    # If no more squares can be filled in, keep making a guesses until you hit a correct one
